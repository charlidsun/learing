...
public class NetUrlConst {
	...
	public static String getChatSearchUrl(String query,String begin,String count){
		String param = "&token="+getToken()+"&query="+query+"&begin="+begin+"&count="+count;
	...
	public static String getArticleUrl(String fakeId,String begin,String count){
		String param = "&token="+getToken() + "&fakeid="+fakeId+"%3D%3D&begin="+begin+"&count="+count;
		return CHAT_ARTICLE_URL + param;
	}

	RedisCacheService redisCacheService = SpringUtil.getBean(RedisCacheService.class);
	public String getToken() {
		return redisCacheService.getToken();
	}	
